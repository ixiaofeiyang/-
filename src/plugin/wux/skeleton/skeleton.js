Component({
    ...
    ready: function() {
        ...
        wx.createSelectorQuery().selectAll(`.${this.data.selector}`).boundingClientRect().exec(function(res) {
            ...
            let height = !!res && res[0] && res[0][0] && res[0][0].height
            let top = !!res && res[0] && res[0][0] && res[0][0].top
            that.setData({
                'systemInfo.height': height + top
            })
        });
